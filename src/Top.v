...
module pipelinedCPU #(parameter FlushAfter=500000)(input clk, input reset);
...
wire [4:0] NEX_RT;
wire [4:0] NEX_SHAMT;
assign NEX_RS = DecodeInitiatesStall?5'b00000:RS; // RS = 0 if hazard
assign NEX_RT = DecodeInitiatesStall?5'b00000:((ID_UseRT_ctrl|ID_memWrite_ctrl)?RT:5'b00000); // RT = 0 if hazard or used IMM
assign NEX_SHAMT = DecodeInitiatesStall?5'b00000:ID_SHAMT;
...
wire [63:0] HILO;        // HILO
wire [63:0] FWD_HILO;    // Bypass in Decode
wire [63:0] EX_HILO;    // Version of HILO available to MFLO in EX


wire [63:0] NextHILO;   // Next HILO

wire [1:0] ID_UseHILO_ctrl;
wire [1:0] EX_UseHILO_ctrl;
...
assign ExecuteInitiatesStall = EX_unknown_ctrl & ~IMEM_READY; // BTB logic requires stalling if the instruction bits aren't available to do the write to BTB
assign DecodeInitiatesStall = HazardDetected | pipeFlush; // Initiates stalls when hazard detected
...
assign ExecuteStall = MemoryStall | ExecuteInitiatesStall; // BTB logic requires stalling if the instruction bits aren't available to do the write to BTB
...
wire [31:0] ICacheFillData;
DummyBypassICACHE thisICACHE(.Address(PC),.InstBits(IBits),.Ready(IMEM_READY),.clk(clk),
.IFill(ICacheFillRequest),
.IFill_Address(ICacheFillAddress),
.Fill_Contents(ICacheFillData));
 
/*
wire [63:0] ICacheFillData;
...
assign BorPCp4Target = (T_branch_mispredicted_NT)?BranchTarget
                        :(NT_branch_mispredicted_T)?ID_PCp4
                            :((BTB_hit&(PredictedBranchDirection | BTB_is_jump))?BTBTargetPC:PCp4); // Branch and branched else PC+4
assign NextPC = (J_mispredicted_NT)? JorRTarget:BorPCp4Target; // Jump side else branch side

assign NextInst = (pipeFlush|IFlush|FetchInitiatesStall)?NOPbits:(((ID_known_ctrl&(ID_PredictedBranchDirection | ID_BTB_Jump)))?BTB_InstBits:IBits);
//assign NextInst = (pipeFlush|IFlush|FetchInitiatesStall)?NOPbits:(IBits);

assign successfulFetch = ~IFlush & ~FetchStall;

// IF-ID Pipeline Register
IF_ID_PR thisIF_ID_PR(.clk(clk),.en(~DecodeStall),.BeingFetched(NextInst),.WasFetched(ID_IBits),.PCp4(PCp4),.ID_PCp4(ID_PCp4),.PC_in(PC),.ID_PC(ID_PC),.BTB_known_ctrl(BTB_hit),.ID_known_ctrl(ID_known_ctrl),.PredictedBranchDirection(PredictedBranchDirection),.ID_PredictedBranchDirection(ID_PredictedBranchDirection),.BTB_is_Jump(BTB_is_jump),.BTB_was_Jump(ID_BTB_Jump));
...
.MEM_memToReg(MEM_memToReg_ctrl),.ID_isBranch(Branch_ctrl),.ID_isJR(RegToPC_ctrl),.EX_WriteHILO_ctrl(EX_WriteHILO_ctrl),.ID_UseHILO_ctrl(|ID_UseHILO_ctrl),
...
assign NRP1Forwarded = DecodeInitiatesStall?32'h0000_0000:RP1Forwarded;
assign NRP2Forwarded = DecodeInitiatesStall?32'h0000_0000:ID_OperandB;
assign NRTVAL = DecodeInitiatesStall?32'h0000_0000:RP2Forwarded;
...
assign IFlush = T_branch_mispredicted_NT|NT_branch_mispredicted_T|J_mispredicted_NT;
...
.DestReg_in(NEX_RegWriteDest),.DestReg_out(EX_RegWriteDest),
    ...
    .PC_in(ID_PC),
    .EX_PC(EX_PC),
    .ID_unknown_ctrlPC(DecodeInitiatesStall?1'b0:(~ID_known_ctrl & (BranchTaken | (Jump_ctrl &~ RegToPC_ctrl) ))),
    .EX_unknown_ctrlPC(EX_unknown_ctrl),
    .ID_Target(Branch_ctrl?BranchTarget:JumpTarget),
    .EX_Target(BTB_UpdateTargetPC),
    .ID_WriteHILO_ctrl(DecodeInitiatesStall?1'b0:ID_WriteHILO_ctrl),
    .EX_WriteHILO_ctrl(EX_WriteHILO_ctrl),
    .ID_UseHILO_ctrl(DecodeInitiatesStall?2'b00:ID_UseHILO_ctrl),
    .EX_UseHILO_ctrl(EX_UseHILO_ctrl),
    .ID_HILO(FWD_HILO),
    .EX_HILO(EX_HILO)
    );
...
EXForwarding thisEXForwarding(.EX_RS(EX_RS), .EX_RT(EX_RT),
    ...
    wire [31:0] FWD_LO;
    assign FWD_LO = WB_WriteHILO_ctrl?WB_ALUOut:EX_HILO[31:0];
    wire [31:0] FWD_HI;
    assign FWD_HI = WB_WriteHILO_ctrl?WB_HI:EX_HILO[63:32];
...
twoCycle_mult thisMult(.clk(clk),.reset(reset),
    .isSigned(EX_ALUOpSelect_ctrl==4'hC),.MCAND_in(ALUPortA),.MPLIER_in(ALUPortB),
    .Product({MEM_HI,MEM_LO}));

// EX-MEM Pipeline Register
EX_MEM_PR thisEX_MEM_PR(.clk(clk),.en(~MemoryStall),
.ALUOut_in((|EX_UseHILO_ctrl)?(EX_UseHILO_ctrl[0]?FWD_LO:FWD_HI):ALUOut),.MEM_ALUOut(MEM_ALUOut),
...
.Link_in(ExecuteInitiatesStall?1'b0:EX_Link_ctrl),.MEM_Link(MEM_Link_ctrl),
...
wire [31:0] DCacheFillData;
wire [31:0] DCacheWritebackData;
                        // The Data Memory (IMEM is not coherent with DMEM)
DummyBypassDCACHE thisDCACHE(.Address(MEM_ALUOut),.SValue(MEM_StoreValue),.RValue(MemOut),.ReadMem(MEM_memToReg_ctrl),.WriteMem(MEM_memWrite_ctrl),.LBU(MEM_LBU_ctrl),.ByteOp(MEM_ByteSize_ctrl),.clk(clk),.Ready(DMEM_READY),
...
/*
wire [63:0] DCacheFillData;
wire [63:0] DCacheWritebackData;
                        // The Data Memory (IMEM is not coherent with DMEM)
...
.ALUOut_in(MemoryInitiatesStall?32'h0000_0000:(MEM_WriteHILO_ctrl?MEM_LO:MEM_ALUOut)),
...
.MEM_WriteHILO_ctrl(MemoryInitiatesStall?1'b0:MEM_WriteHILO_ctrl), .WB_WriteHILO_ctrl(WB_WriteHILO_ctrl),
.MEM_HI(MEM_HI), .WB_HI(WB_HI));
...
JohnnyMnemonic thisMainMemory(.clk(clk),
.IMEM_Read_Address(ICacheFillAddress),.IMEM_Read_Data(ICacheFillData),.IMEM_Read_ctrl(ICacheFillRequest),
.DMEM_Read_Address(DCacheFillAddress),.DMEM_Read_Data(DCacheFillData),.DMEM_Read_ctrl(DCacheFillRequest),
.DMEM_Write_Address(DCacheWritebackAddress),.DMEM_Write_Data(DCacheWritebackData),.DMEM_Write_ctrl(DCacheWriteback));
...
//////////
//////////
/// Logic for emitting checkpoint file
//////////
//////////
...
assign pipeFlush = (currentICount == FlushAfter);
...
begin
if(dumpTrigger)
    begin // dump states to files and quits the simulation
        pcFile=$fopen("pc.rmh");
        memFile=$fopen("mem.rmh");
        HILOFile=$fopen("hilo.rmh");
        regFile=$fopen("reg.rmh");
        $fdisplay(pcFile,"%x %x %x %x",ID_PC[31:24],ID_PC[23:16],ID_PC[15:8],ID_PC[7:0]); // 100th instruction is in Decode - dump its PC
        $fdisplay(HILOFile,"%x %x %x %x\n%x %x %x %x",HILO[63:56],HILO[55:48],HILO[47:40],HILO[39:32],HILO[31:24],HILO[23:16],HILO[15:8],HILO[7:0]); // dump HILO
        ...
        for (memAddr=0; memAddr<65536; memAddr = memAddr+4)
        begin
        $fdisplay(memFile,"%x %x %x %x",((thisMainMemory.MEMCONTENTS[memAddr]===8'bxxxx_xxxx) ?8'h0:thisMainMemory.MEMCONTENTS[memAddr]),
        ((thisMainMemory.MEMCONTENTS[memAddr+1]===8'bxxxx_xxxx) ?8'h0:thisMainMemory.MEMCONTENTS[memAddr+1]),
        ((thisMainMemory.MEMCONTENTS[memAddr+2]===8'bxxxx_xxxx) ?8'h0:thisMainMemory.MEMCONTENTS[memAddr+2]),
        ((thisMainMemory.MEMCONTENTS[memAddr+3]===8'bxxxx_xxxx) ?8'h0:thisMainMemory.MEMCONTENTS[memAddr+3]));
        end
        for (dcacheIndex = 0; dcacheIndex < 32; dcacheIndex = dcacheIndex+1)
        begin
        if(thisDCACHE.Valid0[dcacheIndex]&thisDCACHE.Dirty0[dcacheIndex]) // dirty line
            begin // write dirty lines as diff
            $fdisplay(memFile,"@ %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x",
        ...
        if(thisDCACHE.Valid1[dcacheIndex]&thisDCACHE.Dirty1[dcacheIndex]) // dirty line
            begin
            $fdisplay(memFile,"@ %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x\n%x %x %x %x",
            {thisDCACHE.Tag1[dcacheIndex],dcacheIndex[4:0],5'b00000},
            ((thisDCACHE.Frames1[dcacheIndex][255:248]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][255:248]),
            ((thisDCACHE.Frames1[dcacheIndex][247:240]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][247:240]),
            ((thisDCACHE.Frames1[dcacheIndex][239:232]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][239:232]),
            ((thisDCACHE.Frames1[dcacheIndex][231:224]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][231:224]),
            ((thisDCACHE.Frames1[dcacheIndex][223:216]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][223:216]),
            ((thisDCACHE.Frames1[dcacheIndex][215:208]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][215:208]),
            ((thisDCACHE.Frames1[dcacheIndex][207:200]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][207:200]),
            ((thisDCACHE.Frames1[dcacheIndex][199:192]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][199:192]),
            ((thisDCACHE.Frames1[dcacheIndex][191:184]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][191:184]),
            ((thisDCACHE.Frames1[dcacheIndex][183:176]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][183:176]),
            ((thisDCACHE.Frames1[dcacheIndex][175:168]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][175:168]),
            ((thisDCACHE.Frames1[dcacheIndex][167:160]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][167:160]),
            ((thisDCACHE.Frames1[dcacheIndex][159:152]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][159:152]),
            ((thisDCACHE.Frames1[dcacheIndex][151:144]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][151:144]),
            ((thisDCACHE.Frames1[dcacheIndex][143:136]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][143:136]),
            ((thisDCACHE.Frames1[dcacheIndex][135:128]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][135:128]),
            ((thisDCACHE.Frames1[dcacheIndex][127:120]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][127:120]),
            ((thisDCACHE.Frames1[dcacheIndex][119:112]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][119:112]),
            ((thisDCACHE.Frames1[dcacheIndex][111:104]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][111:104]),
            ((thisDCACHE.Frames1[dcacheIndex][103:96]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][103:96]),
            ((thisDCACHE.Frames1[dcacheIndex][95:88]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][95:88]),
            ((thisDCACHE.Frames1[dcacheIndex][87:80]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][87:80]),
            ((thisDCACHE.Frames1[dcacheIndex][79:72]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][79:72]),
            ((thisDCACHE.Frames1[dcacheIndex][71:64]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][71:64]),
            ((thisDCACHE.Frames1[dcacheIndex][63:56]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][63:56]),
            ((thisDCACHE.Frames1[dcacheIndex][55:48]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][55:48]),
            ((thisDCACHE.Frames1[dcacheIndex][47:40]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][47:40]),
            ((thisDCACHE.Frames1[dcacheIndex][39:32]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][39:32]),
            ((thisDCACHE.Frames1[dcacheIndex][31:24]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][31:24]),
            ((thisDCACHE.Frames1[dcacheIndex][23:16]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][23:16]),
            ((thisDCACHE.Frames1[dcacheIndex][15:8]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][15:8]),
            ((thisDCACHE.Frames1[dcacheIndex][7:0]===8'bxxxx_xxxx) ?8'h0:thisDCACHE.Frames1[dcacheIndex][7:0])
            );
            end
        end
        $fclose(pcFile);
